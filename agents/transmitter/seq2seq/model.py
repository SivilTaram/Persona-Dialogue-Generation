...
from agents.transmitter.utils import Beam
import os
...
class Seq2seqModel(nn.Module):
    ...
    def __init__(self, opt, num_features,
                 padding_idx=0, start_idx=1, end_idx=2, longest_label=1, dict=None):
        super().__init__()
        self.opt = opt

        self.rank = opt['rank_candidates']
        self.attn_type = opt['decoder_attention']
        ...
        rnn_class = Seq2seqModel.RNN_OPTS[opt['rnn_class']]
        self.decoder = Decoder(
            num_features, padding_idx=self.NULL_IDX, rnn_class=rnn_class,
            emb_size=opt['decoder_embed_dim'], hidden_size=opt['decoder_hidden_size'],
            num_layers=opt['decoder_layers'], inp_dropout=opt['decoder_dropout_in'],
            oup_dropout=opt['decoder_dropout_out'],
            share_output=opt['share_decoder_input_output_embed'],
            attn_type=opt['decoder_attention'], attn_length=opt['attention_length'],
            attn_time=opt.get('attention_time'),
            bidir_input=False,
            numsoftmax=opt.get('numsoftmax', 1),
            softmax_layer_bias=opt.get('softmax_layer_bias', False))

        shared_lt = (self.decoder.tgt_word_emb
                     if opt['lookuptable'] in ['enc_dec', 'all'] else None)
        shared_rnn = self.decoder.rnn if opt['rnn_share'] == 'shared' else None

        # turn 30 is enough
        self.encoder = Encoder(
            num_features=num_features, turn_emb_size=opt['encoder_turn_dim'], turn_size=50,
            padding_idx=self.NULL_IDX, rnn_class=rnn_class,
            use_turn=opt['encoder_turn_use'], use_dis=opt['encoder_dis_use'],
            emb_size=opt['encoder_embed_dim'], hidden_size=opt['encoder_hidden_size'],
            num_layers=opt['encoder_layers'], inp_dropout=opt['encoder_dropout_in'],
            oup_dropout=opt['encoder_dropout_out'],
            bidirectional=opt['encoder_bidirectional'],
            shared_lt=shared_lt, shared_rnn=shared_rnn)

        if opt['share_encoder_persona_dialogue']:
            self.persona_encoder = self.encoder
        else:
            # a separate encoder for persona
        ...
        if self.rank:
            self.ranker = Ranker(
                ...
                attn_type=opt['decoder_attention'])
        ...
        # add separate persona memory to generalize more
        self.persona_gate = torch.zeros(opt['encoder_hidden_size'], requires_grad=True)
        self.persona_gate_m = torch.zeros(opt['encoder_hidden_size'], requires_grad=True)
    ...
    def forward(self, src_seq, src_seq_turn=None, src_seq_dis=None, tgt_seq=None, tgt_seq_turn=None, cands=None, valid_cands=None, prev_enc=None,
        ...
        if prev_enc is not None:
            ...
            enc_out, hidden = self.encoder(src_seq, src_seq_turn, src_seq_dis)
            attn_mask = src_seq.ne(0).float() if self.attn_type != 'none' else None
        ...
        if tgt_seq is not None:
            y_in = tgt_seq.narrow(1, 0, tgt_seq.size(1) - 1)
            src_seq = torch.cat([starts, y_in], 1)
            if self.attn_type == 'none':
                preds, score, hidden = self.decoder(src_seq, hidden, enc_out, attn_mask)
                predictions.append(preds)
                scores.append(score)
            else:
                for i in range(tgt_seq.size(1)):
                    xi = src_seq.select(1, i)
            ...
            if beam_size == 1:
                ...
                for _ in range(self.longest_label):
                    ...
                    preds, score, hidden = self.decoder(src_seq, hidden, enc_out, attn_mask, topk)
                    scores.append(score)
                    src_seq = preds
                    predictions.append(preds)
                ...
                src_seq = src_seq.repeat(1, beam_size)
                ...
                src_seq = src_seq.view(bsz * beam_size, -1)
                for step in range(self.longest_label):
                    if all((b.done() for b in beams)):
                        break
                    out = self.decoder(src_seq, hidden, enc_out)
                    scores = out[1]
                    scores = scores.view(bsz, beam_size, -1)  # -1 is a vocab size
                    for i, b in enumerate(beams):
                        b.advance(F.log_softmax(scores[i, :], dim=-1))
                    src_seq = torch.cat([b.get_output_from_current_step() for b in beams]).unsqueeze(-1)
...
class Encoder(nn.Module):
    def __init__(self, num_features, turn_size=0, padding_idx=0, rnn_class='lstm', turn_emb_size=50, use_turn=False,
                 use_dis=False, emb_size=128, hidden_size=128, num_layers=2, inp_dropout=0.1, oup_dropout=0.1,
                 bidirectional=False, shared_lt=None, shared_rnn=None,
                 sparse=False):
        super().__init__()

        self.in_dropout = nn.Dropout(p=inp_dropout)
        self.out_dropout = nn.Dropout(p=oup_dropout)
        self.layers = num_layers
        self.dirs = 2 if bidirectional else 1
        self.hsz = hidden_size

        if shared_lt is None:
            self.src_word_emb = nn.Embedding(num_features, emb_size,
                                             padding_idx=padding_idx,
                                             sparse=sparse)
        else:
            self.src_word_emb = shared_lt

        if use_turn:
            self.turn_state_emb = nn.Embedding(turn_size, turn_emb_size,
                                               padding_idx=padding_idx,
                                               sparse=sparse)

        if use_dis:
            self.dis_state_emb = nn.Embedding(turn_size, turn_emb_size,
                                              padding_idx=padding_idx,
                                              sparse=sparse)

        self.use_turn = use_turn
        self.use_dis = use_dis
        self.use_extra_feature = use_turn or use_dis
        ...
        if input_size != emb_size:
            self.input2emb = nn.Linear(input_size, emb_size, False)

        if shared_rnn is None:
            self.rnn = rnn_class(emb_size, hidden_size, num_layers,
                                 dropout=oup_dropout, batch_first=True,
    ...
    def forward(self, src_seq, src_seq_turn=None, src_seq_dis=None):
        bsz = len(src_seq)

        # embed input tokens
        xes = self.in_dropout(self.src_word_emb(src_seq))

        embedd = [xes]
        if self.use_turn and src_seq_turn is not None:
            embedd.append(self.turn_state_emb(src_seq_turn))
        if self.use_dis and src_seq_dis is not None:
            embedd.append(self.dis_state_emb(src_seq_dis))

        if self.use_extra_feature:
            # batch x seq x dimension
            xes = self.input2emb(torch.cat(embedd, dim=2))

        try:
            x_lens = [x for x in torch.sum((src_seq > 0).int(), dim=1).data]
        ...
        encoder_output = self.out_dropout(encoder_output)
        if self.dirs > 1:
...
class Decoder(nn.Module):
    def __init__(self, num_features, padding_idx=0, rnn_class='lstm',
                 emb_size=128, hidden_size=128, num_layers=2, inp_dropout=0.1, oup_dropout=0.1,
        ...
        self.tgt_word_emb = nn.Embedding(num_features, emb_size, padding_idx=padding_idx,
                                         sparse=sparse)
        self.rnn = rnn_class(emb_size, hidden_size, num_layers,
                             dropout=oup_dropout, batch_first=True)
        ...
        shared_weight = self.tgt_word_emb.weight if share_output else None
    ...
    def forward(self, xs, hidden, encoder_output, attn_mask=None, topk=1):
        xes = self.in_dropout(self.tgt_word_emb(xs))
        ...
        if self.numsoftmax > 1:
            ...
            active = self.out_dropout(self.activation(latent))
            ...
            e = self.out_dropout(self.o2e(output))
            scores = self.e2s(e)
        ...
        if topk == 1:
            ...
            max_score, idx = torch.topk(F.softmax(scores.narrow(2, 1, scores.size(2) - 1), 2), topk, dim=2,
                                        sorted=False)
